...
import com.feieryuu.picturebackend.manager.sharding.DynamicShardingManager;
...
import com.feieryuu.picturebackend.model.enums.SpaceLevelEnum;
...
import com.feieryuu.picturebackend.mapper.SpaceMapper;
...
import static com.feieryuu.picturebackend.exception.ErrorCode.OPERATION_ERROR;
...
//    private DynamicShardingManager dynamicShardingManager;
    ...
    public void validSpace(Space space, boolean add) {
        ...
        if (spaceType!= null && spaceTypeEnum == null) {
            throw new BusinessException(ErrorCode.PARAMS_ERROR, "空间类别不存在a");
        }
    ...
    public long addSpace(SpaceAddRequest spaceAddRequest, User loginUser) {
        ...
        // 针对用户进行加锁 一个用户只能有一个私有空间 以及一个团队空间
        ...
        synchronized (lock) {
            Long newSpaceId = transactionTemplate.execute(status -> {
                boolean exists = this.lambdaQuery().eq(Space::getUserId, userId)
                        ...
                        .exists();
                ThrowUtils.throwIf(exists, ErrorCode.OPERATION_ERROR, "每个用户每类空间只能有一个");
                // 写入数据库
                boolean result = this.save(space);
                ThrowUtils.throwIf(!result, ErrorCode.OPERATION_ERROR);
                //创建成功后 如果是团队空间 则必须关联新增团队成员记录
