...
import com.feieryuu.picturebackend.mapper.SpaceUserMapper;
...
public class SpaceUserServiceImpl extends ServiceImpl<SpaceUserMapper, SpaceUser>
    ...
    @Resource
    @Lazy //懒加载SpaceUserService 否则会报循环依赖错误
    ...
    @Override
    public List<SpaceUserVO> getSpaceUserVOList(List<SpaceUser> spaceUsers, HttpServletRequest request) {
        // 判断输入列表是否为空
        if (CollUtil.isEmpty(spaceUsers)) {
            return Collections.emptyList();
        }
        // 对象列表 => 封装对象列表
        List<SpaceUserVO> spaceUserVOList = spaceUsers.stream().map(SpaceUserVO::objToVo).collect(Collectors.toList());
        // 1. 收集需要关联查询的用户 ID 和空间 ID
        Set<Long> userIdSet = spaceUsers.stream().map(SpaceUser::getUserId).collect(Collectors.toSet());
        Set<Long> spaceIdSet = spaceUsers.stream().map(SpaceUser::getSpaceId).collect(Collectors.toSet());
        // 2. 批量查询用户和空间
